...
| `engine = create_engine(...)` at import time                          | See `database.md` § Anti-patterns.                              | Build engines in lifespan, dispose on shutdown.                                              |
...
| `ORJSONResponse` / `UJSONResponse`                                    | See `core-conventions.md` § Performance.                        | Declare a return type or `response_model`; don't override the response class for speed.      |
...
| Custom middleware that catches `Exception` and returns JSON           | Duplicates `@app.exception_handler`; scattered error shape.     | Use `@app.exception_handler(DomainError)` (see `exception-handlers.md`).                     |
...
| `span.record_exception(e)` in a route or handler                      | See `observability.md` § What NOT to do.                        | `log.exception(...)` inside the active span context.                                         |
...
| Running FastAPI with `--workers N` inside a Kubernetes pod            | See `kubernetes.md` § Process model.                            | One worker per pod, `replicas: N` in the Deployment.                                         |
