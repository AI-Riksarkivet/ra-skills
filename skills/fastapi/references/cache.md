...
Every section below assumes `app.state.redis` is built in lifespan and injected as `RedisDep`. The client construction (`make_redis`), lifespan wiring, and the `get_redis(request: Request)` dependency live in [`redis.md`](redis.md) § One shared client — don't rebuild them here, and never wire the dep as `Depends(lambda r: ...)`: FastAPI turns the lambda's parameter into a required query param.
