...
| `@dataclass` for filter / sort / pagination params                     | No validation, no JSON schema, no FastAPI / Pydantic integration. Project-wide ban.       | `class PaginationParams(BaseModel): ...` — project-wide rule, see `writing-python`.                          |
